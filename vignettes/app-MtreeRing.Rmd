...
+ **A. Singe Segment**
    ...
    This is the default path mode. If the box **Horizontal path** is checked (default), a path is placed along the horizontal direction of the graphics window. To create a path in any direction, uncheck the box **Horizontal path**. Here is an example:

+ **B. Singe Segment with width correction**

    When **Horizontal path** is checked, you can check the box **Inclined tree rings**. In that case, two parallel paths will be created and you can mark two ring borders on the same ring. See this example:
...
3. Create path(s) with mouse clicks
    ...
    5) When you have added all endpoints, a pop-up window appears stating the path creation has been completed successfully. See this example:
