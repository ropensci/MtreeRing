...
```r
rate_limit()
#> Error in rate_limit(): could not find function "rate_limit"
```
...
```r
gists(per_page = 2)
#> Error in gists(per_page = 2): could not find function "gists"
```
...
#> Error in gists(since = "2014-05-26T00:00:00Z", per_page = 2): could not find function "gists"
```
...
#> Error in gists("minepublic", per_page = 2): could not find function "gists"
```
...
#> Error in gist(id = "f1403260eb92f5dfa7e1"): could not find function "gist"
```
...
#> Error in gist_create(file, description = "a new cool gist", browse = FALSE): could not find function "gist_create"
```
...
[1] 0.3229318 0.5933054 0.7778408 0.3898947 0.1309717 0.7501378 0.3206379 0.3379005
'}, browse=FALSE)
#> Error in gist_create(code = {: could not find function "gist_create"
```
...
#> Error in run(file, knitopts = list(quiet = TRUE)) %>% gist_create(browse = FALSE): could not find function "%>%"
```


```
#> Error in gists("minepublic")[[1]] %>% delete: could not find function "%>%"
```
...
```r
gists()[[1]] %>% commits()
#> Error in gists()[[1]] %>% commits(): could not find function "%>%"
```
...
gists(what = "minepublic")[[1]] %>%
  add_files(file) %>%
  update()
#> Error in gists(what = "minepublic")[[1]] %>% add_files(file) %>% update(): could not find function "%>%"
```
...
gists(what = "minepublic")[[1]] %>%
  delete_files(file) %>%
  update()
#> Error in gists(what = "minepublic")[[1]] %>% delete_files(file) %>% update(): could not find function "%>%"
```
...
```r
gists()[[1]] %>% embed()
#> Error in gists()[[1]] %>% embed(): could not find function "%>%"
```
...
#> Error in gist(id = "1642874") %>% forks(per_page = 2): could not find function "%>%"
```
...
```r
g <- gists()
#> Error in gists(): could not find function "gists"
(forked <- g[[ sample(seq_along(g), 1) ]] %>% fork())
#> Error in g[[sample(seq_along(g), 1)]] %>% fork(): could not find function "%>%"
```


```
#> Error in gist(forked$id) %>% delete(): could not find function "%>%"
```
