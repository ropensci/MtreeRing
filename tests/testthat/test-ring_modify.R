...
test_that("ring_modify", {
  ...
  t1 <- ring_detect(ring.data = img1, auto.path = T, seg = 3,
                   method = 'watershed', incline = T)
  dn <- attributes(t1)$seg.dn
  apply(matrix(dn, nrow = 1), 2, dev.off)
  t2 <- ring_detect(ring.data = img1, auto.path = T, seg = 3,
  ...
  t5 <- ring_detect(img1, auto.path = T, method = 'watershed', incline = F)
  ...
  t6 <- ring_detect(img1, auto.path = T, method = 'watershed', incline = T)
