context("nearPith")
##test1
test_that("nearPith returns a dataframe", {
...
test_that("nearPith returns a dataframe", {
  ...
  mock2 <- mock(list(arc = arc, p = 264, t = 10, s = 2), cycle = T)
  stub(nearPith, 'add_path', mock2)
  t2 <- nearPith(img1, T, last.yr = 2015)
...
test_that("nearPith returns a dataframe", {
  ...
  mock2 <- mock(
   ...
   cycle = T)
  stub(nearPith, 'locator', mock2)
  t2 <- nearPith(img1, F, last.yr = 2015)
  dev.off(attributes(img1)$dn)
  expect_is(img1, "magick-image")
  expect_is(t2, "data.frame")
})
...
test_that("nearPith returns a dataframe", {
  ...
  mock2 <- mock(
    ...
    cycle = T)
  stub(nearPith, 'locator', mock2)
  t2 <- nearPith(img1, F, last.yr = NULL)
